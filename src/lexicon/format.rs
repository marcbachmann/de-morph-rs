...
//! ```
//!
//! # Bit layout of the 8-byte analysis record (v3)
//!
...
//!   bits 59-63   reserved          (5 bits, must be 0)
//! ```
//!
//! v3 widens the POS field from 4 to 5 bits to make room for the
//! 17th UPOS tag (`Propn`). 20 bits for `lemma_id` is roughly 7× the
//! current corpus (152k lemmas); when that ceiling is in sight a v4
//! bump can widen the field at the cost of the 5 reserved bits.
...
mod tests {
    ...
    fn record_packs_to_known_layout() {
        // Verify the bit positions documented in the module header (v3).
        let r = AnalysisRecord {
        ...
        // Bits 59-63: reserved        = 0
        // Total mask: 0x07FF_FFFF_FFFF_FFFF
        assert_eq!(u, 0x07FF_FFFF_FFFF_FFFF);
    }

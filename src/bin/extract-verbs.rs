//! Extract German verb analyses from a Wiktionary dump.
//!
//! Mirrors `extract-nouns` in shape and output (JSONL). The schema is
//! the union of all per-POS columns; verb rows leave gender/case/number
//! empty when the cell does not apply (e.g. the infinitive) and fill
...
fn print_usage() {
    eprintln!(
        "extract-verbs — extract German verb analyses from a Wiktionary dump\n\
\n\
Usage:\n\
  cargo run --release --features extractor --bin extract-verbs -- [options]\n\
\n\
Options:\n\
...
fn main() -> Result<()> {
    ...
    for page in reader {
        ...
        for entry in extract_verbs(&page.title, &page.text) {
